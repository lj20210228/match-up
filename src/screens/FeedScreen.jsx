...
export default function FeedScreen({ onMatchPress }) {
  ...
  useEffect(() => {
    ...
    const loadMatches = async () => {
      try {
        ...
        const data = await listMatches(filters);
        ...
        if (isMounted) {
          setError(err.message || "Greška pri učitavanju mečeva");
          setMatches([]);
        }
      } finally {
        if (isMounted) {
  ...
  return (
    <div className="h-full min-h-0 flex flex-col overflow-hidden bg-[#0F0F11]">
      <div className="pt-14 px-5 shrink-0">
        ...
        <div className="bg-[#1C1C1E] border border-white/10 rounded-xl flex items-center gap-2.5 px-4 py-3 mb-3.5">
          <svg
            className="w-4 h-4 text-[#F5F5F3]/30 shrink-0"
            fill="none"
            viewBox="0 0 24 24"
          >
            <circle
      ...
      <div className="px-5 pb-3.5 flex items-center gap-2 shrink-0 overflow-x-auto scrollbar-hidden">
        ...
        {distances.map((distance) => {
          const isActive = maxDistance === distance.value;
      ...
      <div className="flex-1 min-h-0 overflow-y-auto px-5 pb-4 scrollbar-hidden">
        {loading ? (
          ...
          <div className="flex flex-col items-center justify-center h-48 text-center text-[#F5F5F3]/30 space-y-2">
            ...
            <p className="text-xs font-medium m-0">
              Nema mečeva u izabranom radijusu.
            </p>
          </div>
        ) : (
...
function MatchCard({ match, onPress }) {
  const joined = Number(match.joined || 0);
  const total = Number(match.total || 0);

  const pct = total > 0 ? joined / total : 0;
  ...
  return (
    <div
      onClick={onPress}
      className="w-full min-h-[220px] bg-[#1C1C1E] border border-white/10 rounded-2xl p-[18px] cursor-pointer relative overflow-hidden transition-all hover:border-white/20"
    >
      {match.urgent && (
      ...
      <div className="flex items-center justify-between mb-3">
          ...
          <div>
            ...
            <div
              className={`text-xs font-semibold flex items-center gap-1 ${
                match.urgent ? "text-[#D4FF00]" : "text-[#F5F5F3]/50"
              }`}
            >
              {match.urgent && (
                <span className="w-1.5 h-1.5 rounded-full bg-[#D4FF00] animate-pulse" />
              )}
              {match.timeLeft || "Vreme nije navedeno"}
            </div>
          </div>
        </div>
        ...
        >
          {isFull
            ? "Popunjeno"
            : `${spotsLeft} mest${spotsLeft === 1 ? "o" : "a"}`}
        </div>
      </div>
      ...
      <div className="mb-3">
        ...
        <div className="text-xs text-[#F5F5F3]/40 flex items-center gap-1">
          <svg
            className="w-3 h-3 text-current shrink-0"
            fill="currentColor"
            viewBox="0 0 24 24"
          >
      ...
      <div className="mb-3.5">
        <div className="flex justify-between mb-1.5">
          ...
          <span
            className={`text-[11px] font-bold ${
              almostFull ? "text-[#D4FF00]" : "text-[#F5F5F3]/35"
            }`}
          >
            {Math.round(pct * 100)}%
          </span>
        ...
        <div className="bg-white/10 rounded-full h-1 overflow-hidden">
          <div
            className="bg-[#D4FF00] rounded-full h-1 transition-all duration-300"
      ...
      <div className="flex items-center justify-between">
        <div>
          <div className="text-sm font-bold text-[#F5F5F3] tracking-tight">
            {Number(match.pricePerPerson) === 0
              ? "Besplatno"
              : `${Number(match.pricePerPerson || 0).toLocaleString()} ${
                  match.currency || "RSD"
                }`}

            {Number(match.pricePerPerson) > 0 && (
              <span className="font-normal text-[#F5F5F3]/40 text-xs">
                {" "}
                /os.
              </span>
            )}
          ...
          <div className="flex items-center gap-1.5 mt-1">
            <div className="w-[18px] h-[18px] rounded-full bg-[#242426] border border-white/10 flex items-center justify-center text-[7px] font-bold text-[#F5F5F3]">
              {match.hostInitials || "?"}
            </div>

            <span className="text-xs text-[#F5F5F3]/40">
              {match.host || "Organizator"}
            </span>
          </div>
        </div>
        ...
        <button
          ...
          className={`px-5 py-2.5 rounded-full text-xs font-extrabold transition-colors ${
            isFull
              ...
              : "bg-[#D4FF00] text-[#0F0F11] cursor-pointer"
          }`}
        >
